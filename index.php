...
class MUST {
	...
	function addWhenSavingPost($aid) {
		if (self::isNoLocal()) self::uploadA(false, false, $aid); // Fixed, v1.0.6
		else self::upload();
	}
	...
	function pageChangelog() {
		...
		<!-- div -->
			...
			<div>
				<h3>Changlog:</h3>
				<ul>
					<li>= 1.0.6 = Process adjusted</li>
					<li>= 1.0.5 = Fixed: errors appeared when auto-draft enabled</li>
	...
	function uploadA($echo = true, $check = false, $aid = -1) {
		...
		foreach ($data as $val) {
			...
			if ($res == array(array()) || empty($res)) return; // Fixed, v1.0.5
				...
				if ($check) {
					if (function_exists('curl_init')) {
						if ($ourl != ''&&self::isRemoteFileExists($ourl)) {
							...
							self::putPM($val->ID, 'link_'.$key, $ourl); // Fixed, v1.0.5
							$isReady = false;
							$needClear = true;
						}
					}
